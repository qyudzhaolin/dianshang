	...
	function send() {
        ...
        if ($httpCode == "0") {
        	...
        	Logs::err('UmengPush.UmengNotification.send',LOG_FLAG_NORMAL,['向友盟推送通知，返回的数据',"Curl error number:" . $curlErrNo,"Curl error details:" . $curlErr ]);
           	...
           	Logs::err('Demo.Demo.getDemoInfo',LOG_FLAG_NORMAL,['向友盟推送通知，返回的数据',"Http code:" . $httpCode," details:" . $result]);
           	// throw new Exception("Http code:" . $httpCode .  " details:" . $result . "\r\n");
        } else {
        	Logs::info('Demo.Demo.getDemoInfo',LOG_FLAG_NORMAL,['向友盟推送通知，返回的数据'," details:" . $result]);
           	return $result;
        }
    }
