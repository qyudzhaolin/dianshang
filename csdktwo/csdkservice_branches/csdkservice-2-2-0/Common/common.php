 ...
 * @return NULL
 */
function request_core_api($func, $params = array(), $timeout = 13) {
    // import('Curl');
 ...
 * @return NULL
 */
function request_java_api($func, $param = array(), $timeout = 120) {
    # 开始时间
    $begin_time = msectime();
    ...
    $curl->setOpt(CURLOPT_ENCODING, 'gzip');
    $curl->setOpt(CURLOPT_USERAGENT, 'CSDK_WAP_UI_LAYER');
    //$curl->setOpt(CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_1_0 );//开发好后注释掉@author：zhuji@vacn.com.cn

    # 服务配置
    $service_config_all = C( 'SERVICE_CONFIG' );

    # 确认指向服务
    $service = explode( '.', $func );
    if( !isset( $service[2] ) || !isset( $service_config_all[ strtoupper( $service[2] ) ] ) ) {
        return null;
    }

    $service_config = $service_config_all[ strtoupper( $service[2] ) ];
    ...
    $prekey = md5( $params['message'] . substr($service_config['sign_key'], 0, 64) );
    $params['sign'] = strtoupper( md5( $prekey . substr($service_config['sign_key'], 64) ) );

    $requestEntry = $service_config['host'] . ':' . $service_config['port'] . $service_config['host_entry'];
    ...
    if ($curl->error) {
        # 结束时间 失败
        $end_time = msectime();
        \System\Logs::err( '_debug.interactive', LOG_FLAG_ERROR, [$service[2], $func, $param, 0, $curl -> error_message, $end_time - $begin_time] );
        return null;
    }

    $return_data = json_decode($curl->response, true);

    # 结束时间 成功
    $end_time = msectime();
    \System\Logs::info( '_debug.interactive', LOG_FLAG_NORMAL, [$service[2], $func, $param, 1, $return_data, $end_time - $begin_time] );
    return $return_data;
}
 ...
 * 
 */
function sendSmsNotToCore($func, $params, $timeout = 13) {
    // import('Curl');
    $curl = new \Library\Curl();
    $curl->setOpt(CURLOPT_TIMEOUT, $timeout);
    $curl->setOpt(CURLOPT_HTTPHEADER, 0);
    $curl->setOpt(CURLOPT_FRESH_CONNECT, true);
    $curl->setOpt(CURLOPT_ENCODING, 'gzip');
    $curl->setOpt(CURLOPT_USERAGENT, 'CSDKWAP_UI_LAYER');
    $params = array(
        'func' => $func,
        'params' => json_encode($params),
    );
    $params['signKey'] = md5($params['params'] . C('SMS_API_REQUEST_SECKEY') . $params['func']);
    $requestEntry = C('SMS_API_HOST') . C('SMS_API_HOST_ENTRY');
    $curl->post($requestEntry, $params);
    if ($curl->error) {
        return null;
    }
    //$curl->response的json 有时候会存在异常，需要截取一下。。
 ...
 * 处理分城市地区
 */
function disposeExtend($content) {
    $result = array();
    $temp_content = explode('|', $content);
    if(count($temp_content)==1){
 ...
 * 创建支付任务加锁
 */
function create_pay_task($orderSN,$payLimitTime = 60){
    $redis = R();
    $payTaskKey = Api\Common\RedisKeyMap::getCreatePayTask($orderSN);
    $lockRes = $redis->setnx($payTaskKey, 1);
    if ($lockRes) {
            //如果添加锁成功则设置有效时间
        $redis->setTimeout($payTaskKey, $payLimitTime);
        return  true;
    } else {
        return 3022;
    }
 ...
 * 生成批次号
 */
function generate_batch_no($suffix){
        $redis = R();
        $redisKey = 'batch_no';
        if($suffix){
            $redisKey .= "_{$suffix}";
        }
        $redisKey .= "_".date('YmdHis');
        if($redis->exists($redisKey)){
            $redis->set( $redisKey, 0, 60 );
        }
            $key = 'return_batch_no_' . date('YmdHis');
        $num = $redis -> incr( $redisKey );
       ...
       if( $num > 999 ) {
           ...
           $batch_no = date( 'YmdHis' ) . $prefix . str_pad( $num, 3, 0, STR_PAD_LEFT );
           return $batch_no; 
}
