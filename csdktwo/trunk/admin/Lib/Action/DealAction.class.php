    ...
    public function all_index()
    {
        ...
        if (! empty($model)) {
            ...
            foreach ($list as $k => $v) {
                ...
                $list[$k]['deal_button'] = '无此状态值组合';
                
                // “申请中”、“待发布”
                if($v['is_effect'] == 0 && $v['is_publish'] == 1){
                    $list[$k]['is_effect'] = '申请中';
                    $list[$k]['is_publish'] = '待发布';
                ...
                $list[$k]['period_id'] = $GLOBALS['db']->getOne("SELECT period.`name` FROM " . DB_PREFIX . "deal_period AS period ," . DB_PREFIX . "deal AS deal WHERE deal.period_id=period.id and deal.id=" . $v['id'] . "");
                $userinfo = $GLOBALS['db']->getRow("select  mobile,user_name from " . DB_PREFIX . "user    where  id=" . $v['user_id'] . " ");
                $list[$k]['user_id'] = $userinfo['user_name'].'-'.$userinfo['mobile'];
...
//         $version = 1;
    ...
    function deal_event_delete(){
        ...
        // 是否当前轮次
        if($event['investor_record_type'] != 1){
            $this->error("非当前轮次，不能删除！",1);
        }
        
        // 是否绑定投资基金
        if(M("deal_trade_fund_relation")->where("deal_trade_event_id = {$event['id']}")->find()){
            $this->error("该轮次已绑定投资基金，不可删除！",1);
        }
        
        // 拟投资基金(募集基金)是否绑定拟投资项目
        $fund_expectant_deal = M("fund_expectant_deal")
                                ->join("cixi_fund on cixi_fund_expectant_deal.fund_id = cixi_fund.id")
                                ->where("cixi_fund_expectant_deal.deal_id = {$event['deal_id']} and cixi_fund.fund_period = 1")
                                ->find();
        if($fund_expectant_deal){
            $this->error("该轮次已经开始融资【基金：{$fund_expectant_deal['short_name']}】，不可删除！",1);
        }
        
        // 删除融资记录
        $re = M("deal_trade_event")->where("id = {$id}")->delete();
        
        if(false === $re){
            $this->error("删除失败",1);
        }else{
            save_log("删除融资记录成功，项目ID:{$event['deal_id']}",1);
            $this->success("删除成功",1);
        }
    ...
    function deal_event_see(){
        ...
        // 所有磁系基金+管理阶段
        $fund_list = $GLOBALS['db']->getAll("select id,name,short_name from " . DB_PREFIX . "fund where is_csdk_fund=1 and fund_period = 2 and is_delete=1 and status=2 order by id desc"); // 二级地址
        $this->assign("fund_list", $fund_list);
