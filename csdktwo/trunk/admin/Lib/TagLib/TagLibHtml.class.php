...
{//类定义开始
    ...
    public function _editor($attr)
    {
        ...
        $name   	=	$tag['name'];
        $style   	    =	!empty($tag['style'])?$tag['style']:'width:750px; height:200px;';
    ...
    public function _list($attr)
    {
        ...
        foreach($fields as $field) {//显示指定的字段
            ...
            // mod by sunerju 2016-09-28
            // 添加排序的可选功能，对于字段包含NOSORT_ 并且 NOSORT_ 出现在第一个位置的，就不会排序
            if(strpos($property[0],"NOSORT_") === 0){
                $parseStr .= '<a href="javascript:void(0)">'.$showname[0].'</a></th>';
            }else{
