...
//仅能输入数字
$(function () {
	// modify by sunerju 2016-05-16
    $("body").on('keyup',"input[own_type='number']",function () {
          //如果输入非数字，则替换为''
          this.value = this.value.replace(/[^\d]/g, '');
    })
    // add by sunerju 2016-05-16
    $("body").on('keyup',"input[own_type='float']",function () {
    	//如果输入非数字和小数点，则替换为''
    	this.value = this.value.replace(/[^\d.]/g, '');
    })
});
...
//	location.href = CURRENT_URL+"&_sort="+sortType+"&_order="+field+"&";
	location.href = ROOT+"?"+VAR_MODULE+"="+module_name+"&"+VAR_ACTION+"="+action_name+"&_sort="+sortType+"&_order="+field;
}
//添加跳转
function add()
{
