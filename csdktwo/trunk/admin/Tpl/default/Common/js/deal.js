...
function deal_event_see(id,can_edit)

{
	location.href = ROOT + '?m=Deal&a=deal_event_see&id='+id+'&can_edit='+can_edit;
}
...
{   
	...
	$.ajax({ 
		...
		success: function(obj){
			if(obj.status == 1){
				$("#info").html(obj.info.info);
				location.href = ROOT + '?m=Deal&a=deal_completion&id='+obj.info.id;
			}else{
				alert(obj.info);
			}
...
// 删除融资记录
function deal_event_delete(id){   
	if(confirm("确定要删除吗？"))
		$.ajax({
			url: ROOT+"?"+VAR_MODULE+"="+MODULE_NAME+"&"+VAR_ACTION+"=deal_event_delete&id="+id, 
			data: "ajax=1",
			dataType: "json",
			success: function(obj){
				if(obj.status == 1){
