...
// 另外ajax返回的参数名需分别为id,name,与third_data;
  ...
  //基金联想，多传一个基金ID
    function getLinkFund(linkInputId,linkInputfund,actionName,otherInput1,otherInput2) {
        ...
        var linkDatafund = document.getElementById(linkInputfund).value; //获得对应的基金ID
        clearModels();//清除联想输入提示框的内容
        //利用ajax获取后台的模糊查询的数据，并且封装成下拉列表的形式展现出来
        $.ajax({
            // type : "post",//提交的方法为post
            url:ROOT+"?"+VAR_MODULE+"=fund&"+VAR_ACTION+"="+actionName+"&linkValue="+linkDataProperty.value+"&fund_id="+linkDatafund,//对应的moudel方法
