...
class dealdetailsModule extends BaseModule {
	public function index() {
		...
		// 项目基本信息
		$field = "id,img_deal_cover,deal_brief,period_id,province,city,view_count,user_id,profession_info,operation_info,vision_info,is_case,is_effect,entry_info,deal_url,vision_pc_img,profession_pc_img,spot_pc_img";
...
// 		$deal ['city'] = $GLOBALS['db']->getOne("select name from ".DB_PREFIX."region_conf where id={$deal ['city']} and region_level=3");
		...
		//融资计划
         $deal_plan = $GLOBALS ['db']->getRow ( " select period,pe_amount_plan,pe_sell_scale,investor_after_evalute  from " . DB_PREFIX . "deal_trade_event  where deal_id = $id  and investor_record_type=1 order by id desc limit 0,1" );
         if($deal['is_effect'] != 3){
