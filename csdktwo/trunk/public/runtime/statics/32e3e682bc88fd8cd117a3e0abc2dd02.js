...
if($('.header_infi .user').find('a:first-child').html().length>6){
    $('.header_infi .user').find('a:first-child').addClass('setSize');
}else{
    $('.header_infi .user').find('a:first-child').removeClass('setSize');
}
$(function(){
	...
	$("input[type=text],input[type=password],textarea,select").live('focus',function(){
	...
	createUploadForm: function(id, fileElementId, data)
	{
		...
		if(typeof(data) !== 'undefined'){
			$.each(data,function(i,n){
				inputs += '<input type="hidden" name="'+i+'" value=\''+n+'\' />';
			});
		}
		var form = jQuery('<form  action="" method="POST" name="' + formId + '" id="' + formId + '" enctype="multipart/form-data">'+inputs+'</form>'); 
...
function check_input(obj_json,obj,error_id)
 { 			
		...
		// alert(obj.prop("tagName")+value);
			  ...
			  //密码
		      if (obj_json.password==true && !/^[A-Za-z0-9]{6,12}$/.test(value)){
...
$(function(){
	$('.pwd input').focus(function(event) {
		/* Act on the event */
		$(this).attr({
			placeholder: ' '
		});
	});
	$('.pwd input').blur(function(event) {
		/* Act on the event */
		$(this).attr({
			placeholder: '请输入6至12位密码'
		});
	});	
})
