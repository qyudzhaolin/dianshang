...
function getSendSmsTemplate($source,$data){
    switch($source){
        ...
        case 'admin_fund_bind_investor':
            $tpl = "尊敬的投资人，恭喜您已成功认购{$data[0]}基金，您的购买金额为{$data[1]}万，您可通过登录磁斯达克APP查看投资详情。感谢您对磁斯达克平台的信任与支持，谢谢！";
            break;
        case 'admin_finance_finish':
            $tpl = "尊敬的投资人，您购买的{$data[0]}基金已成功投资{$data[1]}项目{$data[2]}，您可以随时登录磁斯达克查看项目的最新资讯及收益情况，谢谢！";
            break;
        case 'admin_intention_list_pass':
            $tpl = "尊敬的投资人，您对{$data[0]}基金发起的“意向申购”申请已经通过！";
            break;
        case 'code':
...
function getSendSmsType($source){
    $typeArr = array(
        ...
        'admin_intention_list_cert'     => 31,  #意向投资人列表->审核通过，修改会员认证通过
        'admin_user_audit'          => 4,       #会员审核
        'admin_fund_bind_investor'  => 5,       #基金绑定投资人
        'admin_intention_confirm'       => 51,  #意向投资人列表->股份确认，股份确认后添加投资人
        'admin_finance_finish'      => 6,       #融资完成
        'admin_intention_list_pass' => 7,       #意向投资人列表->审核通过
    );
