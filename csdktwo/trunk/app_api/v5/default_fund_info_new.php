...
$fund_rate = isset ( $_POST ['fund_rate'] ) ? trim ( $_POST ['fund_rate'] ) : NULL;
// $uid = 1569;
// $fund_id = 2;
...
$fund_sql = "
		    ...
		    and fund.status=2 and fund.fund_period=2 and fund.is_delete=1 and fund.is_delete=1
...
if (! empty ( $result )) {
	foreach ( $result as $key => $val ) {
		...
		if (! empty ( $result_deal_sum )) {
			...
			//基金投资百分比
			if($fund_rate==1){
				$obj->fund_investor_rate=number_format((($result_deal_sum->sum_amount/$val->total_amount)*100),2);
			}
			else{
				$obj->fund_investor_rate=number_format(($result_deal_sum->sum_amount/$val->total_amount)*100);
			}
		}else{
			$obj->fund_investor_amount ="";
			if($fund_rate==1){
			$obj->fund_investor_rate='0.00';
			}
			else{
