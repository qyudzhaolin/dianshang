...
function get_deal_info_x($result, &$deal_info, $uid,$dealword) {
	foreach ( $result as $key => $val ) {
		...
		if ($dealword==1){ 
		//省市
		$province_sql = "select name from cixi_region_conf where id={$val->province} ";
		$province_name = PdbcTemplate::query ( $province_sql, null, PDO::FETCH_OBJ, 1 ); 
		$obj_final->province = is_null ( $province_name->name ) ? "" : $province_name->name;
		
		//关键词
		$case_sql = "select cate.name as cate_name from cixi_deal project, cixi_deal_cate cate, cixi_deal_select_cate selected where selected.deal_id = project.id and selected.cate_id = cate.id and project.id = {$val->deal_id} ";
		...
		$obj_final->deal_cates   = implode ( $obj_deal_cates ); 
		}else{
...
if (! empty ( $result_investing )) {
	if (! empty ( $result_investing )) {
		get_deal_info_x ( $result_investing, $deal_info_investing, $uid,$dealword );
	...
	if ((is_null ( $cate ) && is_null ( $district ) && is_null ( $period_id ) && is_null ( $focus_filter )&& is_null ( $search )) && (! is_null ( $intend ))) {
		...
		if (empty ( $result_check_user_intend )) {
			$obj->r = "您尚未发起任何投资意向，马上去【项目介绍】看看有什么感兴趣的项目";
		} else {
			$obj->r = "没有更多项目";
		}
	}
