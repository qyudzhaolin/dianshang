...
class Fund extends Base{
	...
	/**
	 * 计算基金投资人所投资金，按项目计算收益数据
	 * @param unknown $fundId 基金ID
	 * @param unknown $fund_income_rate_invester LP基金收益分配比例
	 * @param unknown $deal_income_datas
	 */
	public function calcInvestorIncomeOfFundInvestDeal($fundId,$fund_income_rate_invester,$deal_income_datas = []){
		$fundId = isset($fundId)?$fundId:'';
		$fund_income_rate_invester = isset($fund_income_rate_invester)?$fund_income_rate_invester:'';
		if(empty($fundId) || empty($fund_income_rate_invester) || empty($deal_income_datas)){
			if($this->isInTransaction()){
				$this->rollbackTransaction();
			}
			$this->endResponse('',1001);
		...
		$args = ['fund_id'=>$fundId];
		$sql = "SELECT user_id,fund_id,investor_rate
				FROM cixi_user_fund_relation
				WHERE user_type = 1 AND fund_id = :fund_id";
		$invest_users = $this->executeQuery($sql,$args);
		if(count($invest_users) > 0){
			foreach ($invest_users as  $invest_user){
				foreach ($deal_income_datas as $k => $v){
					$fund_income = $v * $invest_user['investor_rate']/100 * $fund_income_rate_invester/100;
					$args = ['fund_id'=>$fundId,'user_id'=>$invest_user['user_id'],'deal_id'=>$k,'fund_income'=>$fund_income];
					$sql = "REPLACE INTO cixi_user_fund_deal_relation(user_id,user_type,fund_id,deal_id,fund_income)
							VALUE(:user_id,1,:fund_id,:deal_id,:fund_income)";
					$this->executeNonQuery($sql,$args);
				}
			}
		}
