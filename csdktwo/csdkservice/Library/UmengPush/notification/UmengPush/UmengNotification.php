...
abstract class UmengNotification {
	...
	function send() {
        ...
        if ($httpCode == "0") {
          	 // Time out
        	Logs::err('UmengPush.UmengNotification.send',LOG_FLAG_NORMAL,['向友盟推送通知，返回的数据',"Curl error number:" . $curlErrNo,"Curl error details:" . $curlErr ]);
        } else if ($httpCode != "200") {
           	// We did send the notifition out and got a non-200 response
           	Logs::err('UmengPush.UmengNotification.send',LOG_FLAG_NORMAL,['向友盟推送通知，返回的数据',"Http code:" . $httpCode," details:" . $result]);
           	// throw new Exception("Http code:" . $httpCode .  " details:" . $result . "\r\n");
        } else {
        	Logs::info('UmengPush.UmengNotification.send',LOG_FLAG_NORMAL,['向友盟推送通知，返回的数据'," details:" . $result]);
           	return $result;
        }
    }
