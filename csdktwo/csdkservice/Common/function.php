...
function L( $msg, $code = 0 ,$fileName = 'log.txt'){
    $error = new System\Error( $msg, $code);
    $error->log($fileName);
}
...
function import( $class, $base_path = LIB_PATH, $ext = EXT ){
    include_once $base_path . $class . $ext;
}
 ...
 * @return mixed
 * @deprecated Server API无法直接获取用户的真实IP地址
 */
...
function N( $name ){
    ...
    // 单例判断
    if( isset( $class[$group.$new_class] ) ) {return $class[$group.$new_class];}
    // 加载实例化判断
    $new_class_name = '\\Api\\' . $group . '\\' . $new_class;
    if( class_exists( $new_class_name, false ) ) {
        $class[$group.$new_class] = new $new_class_name( );
 ...
 * @return void
 */
function push_express($com, $express_number, $order_id, $type = 0,$order_type = 1) {
    return array(); 
}
 ...
 * 生成6位随即数
 */
function rand6($user_type){
    if($user_type == 1){
        $str = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
        $strlen = strlen($str);
        $index = $strlen - 1;
        $code = $str[mt_rand(0,$index)].$str[mt_rand(0,$index)].$str[mt_rand(0,$index)].$str[mt_rand(0,$index)].$str[mt_rand(0,$index)].$str[mt_rand(0,$index)];
    }else{
        $number = '23456789';
        $letters = 'ABCDEFGHJKMNPQRSTUVWXYZ';
        $letter_lengthe = strlen($letters) - 1;
        $code = array(
            $letters[mt_rand(0,$letter_lengthe)],
            $letters[mt_rand(0,$letter_lengthe)],
            $number[mt_rand(0,7)],
            $number[mt_rand(0,7)],
        );
 ...
 * 生成日志函数
 */
function logs($dir='',$content='',$suffix = ''){
        $dir_path  = LOG_PATH."{$dir}/".date('Ym').'/';
        $suffix = empty($suffix) ? '':"_{$suffix}";
        $file_path = $dir_path.date('Ymd').$suffix.'.txt';
        if(!file_exists($file_path)){
            mkdir($dir_path,0777,true);
        }
        file_put_contents($file_path, "[".date('Y-m-d H:i:s')."]:".$content."\r\n",FILE_APPEND);
}

function gatherStatusTemp($uid,$orderId,$muid){
        $uid = $uid + 0;
        $orderId = $orderId + 0;
        $muid = $muid + 0;
        $redis = R(false);

        $checkFlagKeyPath = "userdo:gatherTemp:{$uid}:{$orderId}:{$muid}";
        $isTooOften = $redis->get($checkFlagKeyPath);
        //如果$checkFlagKeyPath有值, 则
        if ($isTooOften !== false) {
            return false;
        }
        $redis->set($checkFlagKeyPath, 1);
        $redis->setTimeout($checkFlagKeyPath,1); //设置过期时间
        //ok, 可以返回序列号啦
        return true;
}
