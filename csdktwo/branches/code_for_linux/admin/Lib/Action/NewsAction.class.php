...
class NewsAction extends CommonAction
{
    public function index()
    {
        ...
        if (trim($_REQUEST['n_publish_state']) != '') {
         ...
         if (trim($_REQUEST['push_status']) != '') {
            $map['push_status'] = $_REQUEST['push_status'];
        }
        // 下拉列表
        ...
        $query_sql = "SELECT id,n_code,n_class,n_cate,n_deal,n_corner,n_channel,n_title,n_brief,n_list_img,n_app_img,n_desc,n_source,n_person,create_time,n_publish_state,push_status FROM cixi_news WHERE 1=1 ";
        if(count($map) > 0){
        	foreach($map as $k=>$v){
        		if(! is_array($v)){
        				if($k == 'n_cate'){
        ...
        foreach ($list as $k => $v) {
            if ($v['n_publish_state'] == '1') {
                $list[$k]['deal_button'] = '<a href="javascript:news_edit(' . $list[$k]['id'] . ')">编辑</a>&nbsp;<a href="javascript: news_delete(' . $list[$k]['id'] . ')">删除</a>&nbsp;<a href="javascript:news_publish(' . $list[$k]['id'] . ')">发布</a>&nbsp;<a href="javascript:news_send(' . $list[$k]['id'] . ')">推送</a>';
            } elseif ($v['n_publish_state'] == '2') {
                
                $list[$k]['deal_button'] = '<a href="javascript:news_cancel(' . $list[$k]['id'] . ')">撤回</a> &nbsp;<a href="javascript:news_send(' . $list[$k]['id'] . ')">推送</a>';
            ...
            $list[$k]['n_publish_state'] = $stateTypeMap[$v['n_publish_state']];
             $list[$k]['push_status'] = $sendTypeMap[$v['push_status']];
            // 初始化列表行业属性值
            $re = explode(',', $v['n_cate']);
    ...
    public function news_send()
    {
         $ids = intval($_REQUEST['id']);
         $condition['id'] = $ids;
         $result = $GLOBALS['db']->getRow("select id,app_version,app_url from " . DB_PREFIX . "app where id=7");
         $url="{$result["app_url"]}/{$result["app_version"]}/appnews/{$ids}";
         $text=$GLOBALS['db']->getOne("select n_title from " . DB_PREFIX . "news where id=$ids");
         $adm_session = es_session::get(md5(conf("AUTH_KEY")));
         $name = $adm_session['adm_name'];
         //发布状态
        ...
        // 开始事物
         M()->startTrans();
         $sate = M(news)->where("id=".$ids)->save($data);
         $params = array(
            text => $text,
            url=>$url
        );
        $func='Common.UmengPush.sendBroadcast';
        $result=request_service_api($func,$params); 
        $resultstatus=$result['status'];
        if ($resultstatus==0) {
               // 提交事务
              M()->commit(); 
            // 成功提示
            save_log($log_info . L("PUSH_SUCCESS"), 1);
            $this->success(L("推送成功"));
        } else {
           ...
           M()->rollback(); 
            ...
            // 错误提示
            save_log($log_info . L("PUSH_FAILED"), 0);
            $this->error(L("推送失败"), 0, $log_info . L("PUSH_FAILED"));
        }
    }
    //轮播首页
