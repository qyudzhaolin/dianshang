...
// 二维数组转一维数组
// 二维数组结构{{id=>0,name=>***},{id=>1,name=>***},...}
// 返回结果： {0=>***,1=>***}
function convert_array($dyadic_array){
	$arr = array();
	if(count($dyadic_array) >=1){
		foreach ($dyadic_array as $v){
			$arr[$v[id]] = $v[name];
		}
	}
	return $arr;
...
// 根据业务对象名称及id值生成业务对象编码
function generalBusinessCode($objName,$id){
	if(empty($id)){
		return 0;
	}
	$business_code_rule = C('BUSINESS_CODE_RULE')[$objName];
	$num = str_pad($id, $business_code_rule[1],0,STR_PAD_LEFT);
	return $business_code_rule[0].$num;
}
