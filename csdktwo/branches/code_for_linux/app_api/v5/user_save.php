...
if (!is_null($img_user_logo)||!is_null($per_degree)||!is_null($user_name)||!is_null($district)||!is_null($img_card_logo) ||!is_null($is_review)) 
{
...
$investor_cate =  isset($_POST['investor_cate'])? trim($_POST['investor_cate']):NULL;
...
$para_value_cate=array();
if (!is_null($investor_cate))
{
	$para_value_cate[] = $investor_cate;
	$para_value_cate[] = $uid;
	$cateidresult = explode('_', $investor_cate, 5);
	
	$delete_cate="delete from cixi_user_select_cate where user_id =$uid" ;
	$delete_result =  PdbcTemplate::execute($delete_cate,NULL);
	if ($uid) {
		$values="";
		...
		$iNSERT_cate="iNSERT INTO cixi_user_select_cate VALUES {$values}" ;
 
		$iNSERT_result =  PdbcTemplate::execute($iNSERT_cate,NULL);
	 
	}
	$sql_cate="update cixi_user_ex_investor set  cate_choose=?   where user_id=?" ;
