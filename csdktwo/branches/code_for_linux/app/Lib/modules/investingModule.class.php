...
class investingModule extends BaseModule{
	...
	public function index(){
		...
		$map    = "log.user_id={$user_id} and log.deal_id=deal.id and deal.is_publish =2";
		$sql 	= "select {$field} from ".DB_PREFIX."deal_intend_log as log,".DB_PREFIX."deal as deal where {$map} order by log.create_time desc limit {$this->rows}";
		$invest = $GLOBALS['db']->getAll($sql);
		$invest_more = 0;	  # 加载更多按钮显示(1)隐藏(0)标识位
		
		if($invest){
    		$invest_count   = $GLOBALS['db']->getOne("select count(1) from ".DB_PREFIX."deal_intend_log as log,".DB_PREFIX."deal as deal where {$map}");
    		$invest_more    = count($invest) < $invest_count ? 1 : 0;
	...
	*/
	public function more(){
		...
		if($user_id){
			...
			$map    = "log.user_id={$user_id} and log.deal_id=deal.id and deal.is_publish =2";
			$sql 	= "select {$field} from ".DB_PREFIX."deal_intend_log as log,".DB_PREFIX."deal as deal where {$map} order by log.create_time desc limit {$counter},{$this->rows}";
			$invest = $GLOBALS['db']->getAll($sql);
...
// 					$v['deal_sign'] 		= msubstr($v['deal_sign'],0,57);
				...
				$invest_count = $GLOBALS['db']->getOne("select count(1) from ".DB_PREFIX."deal_intend_log as log,".DB_PREFIX."deal as deal where {$map}");
				$res['more'] = count($invest) + $counter < $invest_count ? 1 : 0;
				$res['data'] = $invest;
			}else{
				$res['status'] 	= 1;
				$res['info'] 	= "没有数据";
