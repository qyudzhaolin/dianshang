...
class investorsModule extends BaseModule {
	...
	public function index() {
	   ...
	   foreach($carousel as &$v){
		...
		// 成功案列显示
		$dealcase = $GLOBALS ['db']->getAll ( "select {$deal} from " . DB_PREFIX . "deal where is_case = 1 and is_effect=3 and is_publish=2 and is_delete=0 order by sort asc,id desc,create_time desc limit 3" );
		// 精品项目
		$dealbest = $GLOBALS ['db']->getAll ( "select {$dealbest} from " . DB_PREFIX . "deal where is_best = 1 and is_effect=2 and is_publish=2 and is_delete=0 order by sort asc,id desc,create_time desc limit 4" );
		// 项目融资阶段
