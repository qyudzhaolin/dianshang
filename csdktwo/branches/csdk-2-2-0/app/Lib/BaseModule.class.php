...
class BaseModule{
	...
	public function __construct()
	{
		...
		// 如果账户被禁用 清除session
		if($user_info){
			$sql="select is_effect from ".DB_PREFIX."user where id={$user_info['id']}";
		...
		// 需要验证登陆状态的相关模块，权限列表在app_config.php里面配置
		$access = json_decode(APP_LOGIN_STATE_ACCESS,true);
		$ctl 	= strtolower(MODULE_NAME);
		$act 	= strtolower(ACTION_NAME);
		if(isset($access[$ctl]) && in_array($act,$access[$ctl])){
			if(!$user_info){
	...
	public function is_login(){
		...
		if($is_review == 1){
			$side_step['side_step'] = 6;
 			$GLOBALS['db']->autoExecute(DB_PREFIX."user",$side_step,"UPDATE","id=".$id,"SILENT");
		}
	  	$is_exist_sub_user= is_exist(DB_PREFIX."sub_user","user_id={$id}");
	  	//注册步骤数值
		$side_step = $GLOBALS['user_info']['side_step'];
	  	$GLOBALS['tmpl']->assign("side_step",$side_step);
		...
		if(empty($vip_account['vip_money']) || empty($vip_account['vip_begin_time']) || empty($vip_account['vip_end_time'])){
			$is_exist_vip_account=0;
		}else{
			$is_exist_vip_account=1;
		}
		$GLOBALS['tmpl']->assign("is_exist_vip_account",$is_exist_vip_account);
	}
	...
	function _empty(){
		...
		$patt_url_event = "index.php?ctl=";
		$patt_url_event_last ="index.php";
		$patt_url_position = strpos($page_url, $patt_url_event);
		$patt_url_index= "/^\S+index\.php$/";
		$patt_math = preg_match($patt_url_index, $page_url);
		$patt_url_test_index="http://www.test.com/";
		$patt_url_csdk_index="http://csdk.cisdaq.com/";
		$patt_url_csidaq_index="http://cisdaq.com/";		
		 if($page_url === $patt_url_test_index){
		 	$patt_test_event = 1;
		 }
		/* if($page_url === $patt_url_test_index){
		 	$patt_url_csdk_index = 1;
		 }
		 if($page_url === $patt_url_csidaq_index){
		 	$patt_url_csidaq_index = 1;
		 }|| $patt_url_csdk_index == 1 || $patt_url_csidaq_index == 1*/
		if($patt_url_position>0 || $patt_math == 1 || $patt_test_event == 1 ){        	
			$GLOBALS['tmpl']->display("index");  	
		}else{			
			$GLOBALS['tmpl']->display("error404.html");die;		   
		}
