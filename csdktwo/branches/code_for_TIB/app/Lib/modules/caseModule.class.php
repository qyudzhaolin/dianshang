...
FROM cixi_deal_trade_event AS event left join  cixi_deal_trade_fund_relation AS fund on (event.id=fund.deal_trade_event_id),".DB_PREFIX."deal_period as period 
		WHERE `event`.deal_id=$id  and investor_record_type=2   and `event`.deal_id=fund.deal_id and  `event`.period=period.id 
group by event.id
