...
*/    // 分页显示定制
    protected $config  =    array('prev'=>'《','next'=>'》','theme'=>'%prev%  %linkPage%  %next%');
    ...
    public function csdk_show() {
      ...
      /*  $theFirst = "<a href='".$this->get_page_link($url,1)."' >".$this->config['first']."</a>";
        ...
        if ($upRow>0){
            ...
            $upPage="<a href='javascript:void(0)'>".$this->config['prev']."</a>";
        }

        if ($downRow <= $this->totalPages){
            $downPage="<a href='".$this->get_page_link($url,$downRow)."'>".$this->config['next']."</a>";
        }else{
            $downPage="<a href='javascript:void(0)'>".$this->config['next']."</a>";
        }
        $linkPage = "";
        for($i=$this->nowPage;$i<=$this->nowPage+10;$i++){
            $page=($nowCoolPage-1)*$this->rollPage+$i;
            if($page!=$this->nowPage){
                if($page<=$this->totalPages){
                        //$linkPage .= "&nbsp;<a href='".$this->get_page_link($url,$page)."'>&nbsp;".$page."&nbsp;</a>";
        ...
        // 1 2 3 4 5
/*        if($this->totalPages-$this->nowPage>10){
            $linkPage = "";
            for($i=$this->nowPage;$i<=$this->nowPage+10;$i++){
                $page=($nowCoolPage-1)*$this->rollPage+$i;
                if($page!=$this->nowPage){
