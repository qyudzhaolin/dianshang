...
//
	...
	t1.to([$('.mask')],.2,{width:$(document,'body','html').width(),height:$(window).height(),ease:Back.easeOut},'-=.4').from([$('.mask')],.7,{opacity:0,zIndex:0,ease:Back.easeOut},'-=.2').from([$('.text_div')],.4,{scale:0,ease:Linear.easeInOut},'-=.2').from([$('.b')],1,{top:200,ease:Back.easeOut},'-=.3').from([$('.box')],1,{top:300,ease:Back.easeOut},'-=0.8');
