...
if (! empty ( $result )) {
	foreach ( $result as $key => $val ) {
		...
		// 基金总投资金额
		$sql_fund_sum = "
       		 select sum(investor_amount) as sum_amount from cixi_fund_deal_relation as relation,cixi_deal as deal where deal.id=relation.deal_id and fund_id=" . $obj->fund_id . "  ";
		...
		// 基金投资项目
		$sql_fund_deal = "
                select deal_id,investor_amount,investor_date,deal.s_name from cixi_fund_deal_relation as relation,cixi_deal as deal where deal.id=relation.deal_id and deal.is_effect in (2,3) and fund_id=" . $obj->fund_id . " order by investor_amount desc ,investor_date desc ,relation.id desc ";
