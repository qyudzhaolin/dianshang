...
{//类定义开始
    ...
    static function save($type=self::FILE,$destination='',$extra='')
    {
        ...
        {
        	if(!is_dir( APP_ROOT_PATH.app_conf("LOG_PATH")))
        	{
        		if(!mkdir(APP_ROOT_PATH.app_conf("LOG_PATH")))
        		{
        			return false;
        		}
        	}
            $destination = APP_ROOT_PATH.app_conf("LOG_PATH").date('y_m_d').".logger";
        }
    ...
    static function write($message='',$level=self::ERR,$type=self::FILE,$destination='',$extra='')
    {
        ...
        if(empty($message)){
            $message = json_encode(error_get_last(), JSON_UNESCAPED_UNICODE);
