...
{	
    ...
    static function get($name) {
    	if(isset($_SESSION[app_conf("AUTH_KEY").$name."expire"]) && $_SESSION[app_conf("AUTH_KEY").$name."expire"] > time()){
	    	if(isset($_SESSION[app_conf("AUTH_KEY").$name])){
				 $value   = $_SESSION[app_conf("AUTH_KEY").$name];
				 self::set($name,$value); // 活跃时，保持登录状态
			} else{
				$value = null;
			}
    	}else{
    		es_session::clear_one_session(app_conf("AUTH_KEY").$name);
    		$value = null;
    	}
	...
	// 设置带有效期的某个session值
    static function set($name,$value,$expire=1440) {
//     	if(empty($expire)){
//     		$expire=1440;
//     	}
        $_SESSION[app_conf("AUTH_KEY").$name]  =   $value;
        $_SESSION[app_conf("AUTH_KEY").$name."expire"] = time() + $expire;
        // var_dump(time() + $expire);
    }
    ...
    // 删除某个session值
    static function clear_one_session($name){
    	unset($_SESSION[$name]); 
