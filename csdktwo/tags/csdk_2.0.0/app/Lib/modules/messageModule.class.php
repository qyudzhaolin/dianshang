...
class messageModule extends BaseModule
{
	...
	{	
		...
		$msg_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."user_notify  where user_id = ".intval($GLOBALS['user_info']['id'])." order by id desc");
		foreach ($msg_list as $key => $value) {
		 	$msg_list[$key]['log_time']=date("Y.m.d",$value['log_time']);
		} 
		$user_notify['is_read']=1;
		$result=$GLOBALS['db']->autoExecute(DB_PREFIX."user_notify",$user_notify,"UPDATE","user_id=".$GLOBALS['user_info']['id'],"SILENT");
		$user_notify_count=0;
