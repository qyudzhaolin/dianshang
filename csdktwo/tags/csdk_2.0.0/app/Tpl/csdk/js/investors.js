...
$(function(){
		...
		var status=true;
		$('.filtrate_box_invest').click(function(){
			if(status){
				$('.deals_box').show();
				$('.filtrate').hide();
				status=false;
	...
	$("#three_row li").each(function(){
     ...
     //提交
        function deals_post(){
            ...
            var url= APP_ROOT+"/index.php?ctl=investors&act=index"+periods+cates+district;
            location.href = url;
            //alert(url);
...
function next_page(page_num){
	if($("#cates_all").attr("checked")) {
            ...
            var url= APP_ROOT+"/index.php?ctl=investors&act=index&page="+page_num+periods+cates+district;
            location.href = url;
            //alert(url);
}
