...
function check_tel($tel){
	...
	if(empty($tel))
	{
		$data['status']=0;
		$data['info']=$field_name."不能为空";
	}elseif(!preg_match("/^1[34578]\d{9}$/",$tel)&&!preg_match("/^([0-9]{3,4}-)?[0-9]{7,8}$/",$tel)){
		$data['status']=0;
		$data['info']=$field_name."格式不正确";
	}
	else{
...
function check_pwd($pwd){
	...
	if(empty($pwd))
	{
		$data['status']=0;
		$data['info']=$field_name."不能为空";
	}elseif(!preg_match("/^[A-Za-z0-9]{6,12}$/",$pwd)){
		$data['status']=0;
		$data['info']=$field_name."格式不正确";
	}
	else{
...
function check_url($str){
	...
	if(empty($str))
	{
		$data['status']=0;
		$data['info']=$field_name."不能为空";
	}elseif(!preg_match("/^(https?:\/\/)?(((www\.)?[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)?\.([a-zA-Z]+))|(([0-1]?[0-9]?[0-9]|2[0-5][0-5])\.([0-1]?[0-9]?[0-9]|2[0-5][0-5])\.([0-1]?[0-9]?[0-9]|2[0-5][0-5])\.([0-1]?[0-9]?[0-9]|2[0-5][0-5]))(\:\d{0,4})?)(\/[\w- .\%&=]*)?$/i",$str)){
		$data['status']=0;
		$data['info']=$field_name."格式不正确";
	}
	else{
...
function url($route="index",$param=array())
{
	...
	else
	{
		...
		if(!strstr($url,'about')) $url=preg_replace('/(\d+)/','${1}/',$url);
 		$replace = array('id-','6-g','about/4','about/5','about/6-','home','home//','deals','deals//','intend-1-p','/intend-1/-p-1/','investors','investors//','download','message','case','case//','vip_account_introduction/source-1/','/auth/investor_show','/auth/investor_show/side_step-5/side_step-5');
		$subject =array('','6-','about/4/','about/5/','about/6/','home/','home/','deals/','deals/','intend/-p-1','/intend-1/-p-1','investors/','investors/','download/','message/','case/','case/', 'vip_account_introduction/source-1','/auth/investor_show/side_step-5','/auth/investor_show/side_step-5');
