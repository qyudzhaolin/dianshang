...
function checkPeriod() {
	var period_check = $("#period_id").val();
...
function add_deal_check(add_or_edit) {
	...
	if (add_or_edit == 'add') {
		...
		var check_json = '[\
			...
			{"name":"deal_operation_steps_event","box":1,"required":true,"error_id":"deal_operation_steps_msg",\
					"input":[{"name":"opera_steps_name[]","required":true,"error_msg":"必填"},\
	         ...
	         {"name":"deal_data_img_event","box":1,"required":true,"error_id":"deal_data_img_event_msg",\
					"input":[{"name":"img_data_url[]","required":true,"error_msg":"必填"}]},\
			]';
	}
	...
	var img_deal_logo_check = check_deal_field('img_deal_logo', 1, 0,
			'avatar_file');
	if($('#is_case').val() == 1){
	...
	if (!deal_url_check || !com_web || !province_check
			|| !img_deal_logo_check || !check_all || !url_check|| !com_web_check || !cate_check
			|| !period_check || !user_check || !vis_check || !is_best_check || !capital_check || !city_check) {
		return false;
	}
...
$(document).ready(
		function() {
			$('input[type=text],input[type=password],textarea').live('blur',
					function() {
						var v = $(this).val();
						if (v != '') {
