...
Calendar.prototype.callHandler = function () {
	if (this.onSelected) {
		 if($(this.sel).attr('type')=='text')
		 {
		 	   this.onSelected(this, this.date.print(this.dateFormat));
		 }
		 else
		 {
		      $(this.sel).prev().val(this.date.print(this.dateFormat));
		      $(this.sel).prev().removeClass('default_font');
		      $(this.sel).prev().removeClass('warnning');
		 }
	}
};
...
// 刘斯玉 2015-7-27
function  csdk_cal(obj)
{
	return showCalendar(obj, '%Y-%m-%d', false, false,obj);
}
...
function showCalendar(id, format, showsTime, showsOtherMonths, selbtn) {
  ...
  // to the button.
  //_dynarch_popupCalendar.showAtElement(el.previousSibling, "Br");        // show the calendar
  _dynarch_popupCalendar.showAtElement(btn_el, "Br");
  //$(id).prev().val(el.value);
  return false;
}
