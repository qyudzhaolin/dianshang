...
if(!is_null($deal_id)&&!is_null($uid)){
	...
	if (!empty($result)) {
		$sql_update="update cixi_deal set focus_count = focus_count + 1 where id = ?";
		...
		$para_value_update_user[]=$uid;
		$result_update =  PdbcTemplate::execute($sql_update,$para_value_update_user);
		if ($result_update[0]===false) {
			$obj->r = "关注失败";
			CommonUtil::return_info($obj);
			return;
