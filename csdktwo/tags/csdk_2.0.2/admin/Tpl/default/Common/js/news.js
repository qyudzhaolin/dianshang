...
function message_add() {
	location.href = ROOT + '?m=News&a=message_add';

}

function message_edit(message_id) {

	location.href = ROOT + '?m=News&a=message_edit&id=' + message_id;

}
function message_del(message_id) {
if(confirm("确定要删除吗？"))
$.ajax({ 
			url:  ROOT + '?m=News&a=message_del&id=' + message_id,
			data: "ajax=1",
			dataType: "json",
			success: function(obj){
				$("#info").html(obj.info);
...
function add_message_check() {
	...
	var add_message_json='[{"name":"title","required":true,"error_id":"title_msg","error_msg":"请填写标题"},\
                              {"name":"create_time","required":true,"error_id":"create_time_msg","error_msg":"请选择创建时间"}]';
	var res = csdk_check(add_message_json);
	if (!res ) {
...
function edit_message_check() {

	var edit_message_check ='[{"name":"title","required":true,"error_id":"title_msg","error_msg":"请填写标题"},\
                              {"name":"create_time","required":true,"error_id":"create_time_msg","error_msg":"请选择创建时间"}]';
	var res = csdk_check(edit_message_check);
	if (!res ) {
		return false;
	} else {
		return true;
...
function message_http (id) {
	window.open(API_DOMAIN+"/push/"+id);           
}
