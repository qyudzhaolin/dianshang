...
//         )));
    ...
    function deal_completion_check()
    {
        ...
        if(!empty($deal_id)){
            ...
            $event = M('deal_trade_event')->where("deal_id={$deal_id} and investor_record_type=1")->find();
           
            // 判断有无投资基金
            if(M('deal_trade_fund_relation')->where('deal_trade_event_id='.$event['id'])->find()){
                $this->success(l("success"),1);
            }else{
    ...
    function do_deal_completion()
    {
        ...
        if($trade){
            ...
            if($fund_id){
                ...
                // 查询基金的所有投资人
                $user_id = M('user_fund_relation')->where('fund_id in('.$fund_id.')')->getField('id,user_id');
                if($user_id){
