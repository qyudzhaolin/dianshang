		...
		if (! empty ( $model )) {
			...
			foreach($list as $k=>$v){
				if($v['status']==1){
					...
					$list[$k]['button_list']='<a href="javascript:edit('.$list[$k]['id'].')">编辑</a>&nbsp;<a href="javascript: del('.$list[$k]['id'].')">删除</a>&nbsp;<a href="javascript: fund_investor('.$list[$k]['id'].')">管理基金投资人</a>&nbsp;<a href="javascript: investment_details('.$list[$k]['id'].')">投资详情</a>&nbsp;<a href="javascript: fund_detail('.$list[$k]['id'].')">基金详情</a>';
				}elseif($v['status']==2){
					$list[$k]['status_value']='已发布';
					$list[$k]['button_list']='<a href="javascript: fund_investor('.$list[$k]['id'].')">管理基金投资人</a>&nbsp;<a href="javascript: investment_details('.$list[$k]['id'].')">投资详情</a>&nbsp;<a href="javascript: fund_detail('.$list[$k]['id'].')">基金详情</a>&nbsp;<a href="javascript: revoke('.$list[$k]['id'].')">撤回</a>';
				}elseif($v['status']==3){
...
public function find_investor() {
	...
	public function investor_insert() {
	  ...
	  //var_dump($_REQUEST['team_name']);die();
		...
		$data['is_default_fund'] = 1;
 		
		// 是否为投资人默认选中基金
		if(M('user_fund_relation')->where('user_id='.$data['user_id'])->find()){
		...
		if (false !== $list) {
			//成功提示
 			...
 			$fund_name  = M('fund')->where('id='.$data['fund_id'])->getField('short_name');
 			$SMS        = new csdk_sms();
 			$msg        = "尊敬的投资人，恭喜您已成功认购{$fund_name}基金，您的购买金额为{$data['investor_amount']}万，占比{$data['investor_rate']}%，您可通过登录磁斯达克APP查看投资详情。感谢您对磁斯达克平台的信任与支持，谢谢！";
 			$result     = $SMS->sendSMS($_REQUEST['user_mobile'],$msg,array('type'=>csdk_sms::$TYPE_FUND_BIND_INVESTOR));
 			// 发送站内信
	...
	{	
		if(trim($_REQUEST['s_name']) != ''){
			$map['s_name'] = trim($_REQUEST['s_name']);
		}
		if(trim($_REQUEST['name']) != ''){
			$map['name'] = trim($_REQUEST['name']);
		}
		$id = intval($_REQUEST['fund_id']);
         $list_sql = "select trade.id as id, fund_id,trade.deal_id as deal_id,trade.investor_amount as investor_amount,deal_trade_event_id,investor_date,remark,deal.id as dealid,name,s_name,period"
					." from ".DB_PREFIX."deal_trade_fund_relation as trade,".DB_PREFIX."deal as deal,".DB_PREFIX."deal_trade_event as period "
					." where fund_id=$id and trade.deal_id=deal.id and trade.deal_trade_event_id=period.id and  period.investor_record_type=2 and deal.is_publish=2 and not is_effect=1    ";
					
		$count_sql = "select count(1) "
					." from ".DB_PREFIX."deal_trade_fund_relation as trade,".DB_PREFIX."deal as deal,".DB_PREFIX."deal_trade_event as period "
					." where fund_id=$id and trade.deal_id=deal.id and trade.deal_trade_event_id=period.id and  period.investor_record_type=2 and deal.is_publish=2 and not is_effect=1   ";
         if($map['s_name'] != ''){
			$list_sql .= " and s_name like '%".$map['s_name']."%' ";	
			$count_sql .= " and s_name like '%".$map['s_name']."%' ";	
		}
		if($map['name'] != ''){
			$list_sql .= " and name like '%".$map['name']."%' ";			
			$count_sql .= " and name like '%".$map['name']."%' ";	
		}
		if (method_exists ( $this, '_filter' )) {
			$this->_filter ( $map );
		}
		$result = $this->_list_multi_table($count_sql, $list_sql, $map,"trade.investor_amount ");
		...
		foreach ($result as $k=>$v){
			...
			$result [$k] ['period'] = $periodMap [$v ['period']];
