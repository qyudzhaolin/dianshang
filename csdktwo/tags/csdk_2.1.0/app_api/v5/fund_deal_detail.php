...
FROM cixi_deal_trade_event AS event,cixi_deal_trade_fund_relation as fund,cixi_deal_period as period 

WHERE 
`event`.deal_id=?  and is_csdk_fund=1 and deal_trade_event_id=`event`.id and fund_id=?
...
$deal_sql="
			 ...
			 FROM cixi_deal AS deal  WHERE id=?  ";
$deal_final[] = $deal_id;
...
$csdk_sql="
		select 
		sum(investor_amount) as csdk_investor_amount 
        from 
		cixi_deal as deal left join cixi_deal_trade_fund_relation as relation on deal.id=relation.deal_id
		where   
        is_csdk_fund=1
        and deal.id=event.deal_id and relation.deal_trade_event_id=event.id 
	    and deal.id=? and fund_id=?";
...
$newest_sql="
			...
			FROM 
		    cixi_deal_trade_event AS event  
		    , cixi_deal_trade_fund_relation as trade
		    ,cixi_deal_period as period
			WHERE 
		    `event`.deal_id=?  and event.id=trade.deal_trade_event_id  
			and  `event`.period=period.id
			ORDER BY period.sort desc,event.id DESC LIMIT 0,1";
...
$lastest_select="
			...
			WHERE 
		    `event`.deal_id=?  
		    and trade.fund_id=?  and event.id=trade.deal_trade_event_id
			and `event`.period=period.id
";
...
$event_list_sql="
				...
				and `event`.period=period.id
				group by event.id
				ORDER BY period.sort asc,id asc ";
...
$r_cate_news = PdbcTemplate::query($sql_cate_news,$para_cate_news);
	...
	if(!empty($result_deal))
	{
		foreach ($result_event as $key => $val) 
		{
			...
			/**********获取此项目本轮次融资投资总额******************/
			$trade_sql="
					select sum(investor_amount)  as csdk_investor_amount from cixi_deal_trade_fund_relation as fund , cixi_deal_trade_event as deal where  period=? and deal.deal_id=? and fund.deal_id=deal.deal_id and fund.deal_trade_event_id=deal.id";
