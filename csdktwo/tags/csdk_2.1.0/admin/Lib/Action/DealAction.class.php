					...
					if($fund_id){
						...
						if($user_fid){
							...
							foreach($uf as $kv=>$uv){
    							...
    							$msg        = "尊敬的投资人，您购买的".implode('，',$fund_name)."基金已成功投资{$deal_name}项目{$period}，您可以随时登录磁斯达克查看项目的最新资讯及收益情况，谢谢！";
    							// 发送站内信
								$user_notify = array(
										'user_id'   => $kv,
										'log_info'  => $msg,
								...
								$b = $SMS->sendSMS($mobile,$msg,array('type'=>csdk_sms::$TYPE_FINANCE_FINISH));
							}
						}
					}else{
						// 更新项目融资轮次
