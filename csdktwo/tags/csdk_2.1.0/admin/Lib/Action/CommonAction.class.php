		...
		if ($count > 0) {
			...
			/*if ($model->name=='Deal') {
				foreach($voList as $k=>$v){
					if($v['is_effect']=='1'){
						$voList[$k]['is_effect_zh']='已认证';
					}else{
						$voList[$k]['is_effect_zh']='未认证';
					}
				}
			}
			if ($model->name=='User') {
				foreach($voList as $k=>$v){
					...
					if($v['is_review']=='1'){
						$voList[$k]['is_review_zh']='已审核';
					}else{
						$voList[$k]['is_review_zh']='未审核';
					}
				}
			}*/
//			echo $model->getlastsql();
	...
	// $count_sql 查询总条数SQL，$list_sql 查询数据的SQL，$map 页面检索条件
	protected function _list_multi_table($count_sql,$list_sql,$map,$order){
		// 查询总条数
		...
		if($count > 0){
			...
			$p = new Page($count, $listRows);
			if(!empty($order)){
			$list_sql .=" order by $order  desc "; 
			}
			$list_sql .= "limit $p->firstRow,$p->listRows";
			$voList = $GLOBALS['db']->getAll($list_sql);
