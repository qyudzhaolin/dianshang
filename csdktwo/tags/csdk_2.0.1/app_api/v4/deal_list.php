...
$sql_select = "
				...
				";
/*******************************/
$sql_select_invested = ",investor_amount";
$sql_select_investing = ",pe_amount_plan";
/*******************************/


$sql_from = "from cixi_deal";
$sql_from_invested  = " FROM (
					SELECT * FROM CIXI_DEAL AS DEAL RIGHT JOIN 
					(SELECT A.ID INVST_RC_ID, A.DEAL_ID DEAL_ID
								,B.ID INVST_ORG_ID, B.INVESTOR_AMOUNT FROM CIXI_DEAL_TRADE_EVENT A, CIXI_DEAL_EVENT_INVESTOR B
					WHERE A.ID = B.EVENT_ID
					AND A.IS_CSDK_PARTAKE = 1 -- is csdk participate
					AND B.IS_CSDK_PARTAKE = 1) AS AMOUNT
					ON DEAL.ID = AMOUNT.DEAL_ID
					) AS FINAL_RECORDS
					";
$sql_where = "
				where is_effect = 2
...
/*$sql_order_by_condition = "
							  and is_delete=0 order by sort asc,is_effect asc, update_time desc
							";*/
$sql_order_by_condition = "
							  order by sort asc, pe_amount_plan desc, update_time desc";
$sql_order_by_condition_invested = "
							  order by sort asc, investor_amount desc, update_time desc";
...
if (! is_null ( $period_id )) {
	...
	$para_period_id	= array($period_id);
...
if (! is_null ( $focus_filter )) {
	$sql_order_by_condition = "order by comment_count desc,create_time desc";
	$sql_order_by_condition_invested="order by comment_count desc,create_time desc";
	
}
if (! is_null ( $update_time )) {
	$sql_order_by_condition = "order by update_time desc,create_time desc";
	$sql_order_by_condition_invested="order by update_time desc,create_time desc";
}
if (! is_null ( $search )) {
...
$sql_final_investing = "";
$sql_final_invested = "";
$sql_case_common = "or ( is_case = 1 and is_effect = 3 ";
...
//var_dump($sql_case_common);
//return;



$sql_final_investing = $sql_select . " " . $sql_select_investing . " " . $sql_from . " " . $sql_where . " " . $condition . " " .$sql_order_by_condition;
$sql_final_invested = $sql_select . " " . $sql_select_invested . " " . $sql_from_invested . " " . $sql_where_invested . " " . $condition . " " . $sql_case_common . " "  . $sql_order_by_condition_invested; 
...
$result_invested = PdbcTemplate::query ( $sql_final_invested, $para_final_invt_case, PDO::FETCH_OBJ, 0 );
//$deal_info = array ();
$deal_info_final = array ();
$deal_info_investing = array ();
$deal_info_invested = array ();
...
if(! empty ( $result_investing ) || ! empty ( $result_invested ))
{
	if (! empty ( $result_investing )) 
{
	get_deal_info_x($result_investing, $deal_info_investing, $uid);
	//array_push ( $deal_info_final, $deal_info_investing );
	for($i=0; $i< count($deal_info_investing);$i++)
	{
		$deal_info_final[] = $deal_info_investing[$i];
	}
} 
	if(! empty ( $result_invested )) 
{
	get_deal_info_x($result_invested, $deal_info_invested, $uid);
	for($i=0; $i< count($deal_info_invested);$i++)
	{
		$deal_info_final[] = $deal_info_invested[$i];
	}
} 
	 $obj->status = 200;
...
else if( empty ( $result_investing ) && empty ( $result_invested )){
//筛选列表
			
			
			if((!is_null($cate)||!is_null($district)||!is_null($period_id)||!is_null($focus_filter)) && (!is_null($intend)))
			{
				$obj->r = "没有更多项目";
			}
