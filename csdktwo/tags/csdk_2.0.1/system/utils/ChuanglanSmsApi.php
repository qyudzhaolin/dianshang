...
function sendSMS($mobile, $msg, $needstatus = 'false', $product = '', $extno = '') {
    ...
    $service = new ChuanglanSmsApi();
    
    if(empty($mobile)){
        $result = array('',107);
    }else{
        $result = $service->sendSMS($mobile, $msg, $needstatus, $product, $extno);
        $result = $service->execResult($result);
    }
    
    $result['info'] = $service->getSmsSubmit($result[1]);
...
function queryBalance() {
    ...
    $result     = $service->execResult($result);
    $result['info'] = $service->getBalanceQuery($result[1]);
