...
class investedModule extends BaseModule{
	...
	*/
	...
	public function invested_fund(){
		...
		// 基金已投资金额
		$amount_total = $GLOBALS['db']->getOne("select sum(`investor_amount`) from ".DB_PREFIX."fund_deal_relation where fund_id={$id}");
		...
		$sql    = $sql . $map . " order by r.investor_amount desc,r.investor_date desc limit {$this->invest_deal_rows}";
...
// 		$deal_id =  isset($_GET['deal_id'])? intval($_GET['deal_id']) : 0;
		...
		// 项目信息
		$field = "id,name,s_name,company_name,com_addr,com_busi,com_legal,com_tel,com_web,com_reg_found,com_time,recommend_reason,out_plan,entry_info";
		$deal_info = $GLOBALS['db']->getRow("select {$field} from ".DB_PREFIX."deal where id={$deal_id} and is_effect in(2,3)");
		
		// 查询磁斯达克最新融资信息->获取 投资前估值,投资后估值,该轮融资总额 等数据
		$info_csdk = $GLOBALS['db']->getRow("select id,period,investor_amount,investor_before_evalute,investor_after_evalute from ".DB_PREFIX."deal_trade_event where deal_id={$deal_id} and partake_fund={$fund_id} and is_csdk_partake=1 ORDER BY investor_time desc,id DESC LIMIT 1");
		...
		if($info_csdk){
		    ...
		    $info_csdk['investor_amount']         = number_format($info_csdk['investor_amount']);
    		// 查询磁斯达克最新融资投资金额（当前基金+当前项目+CSDK参与下的最新融资 => 再查询投资记录，拿出投资总额）
    		$enent_id = $info_csdk['id'];
    		$info_csdk['csdk_investor_amount']    = number_format($GLOBALS['db']->getOne("select investor_amount from ".DB_PREFIX."deal_event_investor where event_id={$enent_id} and is_csdk_partake=1 ORDER BY create_time desc,id DESC LIMIT 1"));
		}else{
		...
		// 融资信息
		$trade = $GLOBALS['db']->getAll("select id,investor_time,period,investor_amount,investor_before_evalute,investor_after_evalute,investor_rate,evalute_growth_rate,is_csdk_partake from ".DB_PREFIX."deal_trade_event where deal_id={$deal_id} order by investor_time");
		if($trade){
			foreach($trade as $k=>$v){
				$chart['period']    .= "'{$v['period']}', ";
				$chart['evalute']   .= $v['investor_after_evalute'].", ";
				$chart['amount']    .= $v['investor_amount'].", ";
				$trade[$k]['event'] = $GLOBALS['db']->getAll("select name,s_name,investor_amount,investor_rate from cixi_deal_event_investor where  event_id={$v['id']} order by create_time desc ,investor_amount desc ,id desc");
			}
		}
		// 核心团队 新闻动态（接口请求）
...
// 			}
		...
		if($flag == "news"){
			...
			// 行业新闻
			$cate_id 	= $GLOBALS['db']->getCol("select cate_id from ".DB_PREFIX."deal_select_cate where deal_id={$deal_id}");
			$cate_id 	= implode(",",$cate_id);	# 组装该项目所属行业ID
			if($newsId){
			...
			$news_cate 	= $GLOBALS['db']->getAll("select id,n_title,create_time from cixi_news where {$map} n_cate in ({$cate_id}) and n_channel in ({$channel[1]['id']},{$channel[2]['id']}) and n_publish_state=2 order by id desc LIMIT 0,10");
			if($news_cate){
