...
KE.setting = {
	...
	items : [
		...
		'italic', 'underline', 'strikethrough', 'removeformat', 'image', 'fimage',
		'flash', 'media', 'table', 'hr', 'emoticons', 'link'
	],
	colorTable : [
...
KE.format = {
	getUrl : function(url, mode, host, pathname) {
		...
		if (mode == 'relative') {
			...
			var getRelativePath = function(path, depth) {
				if (url.substr(0, path.length) === path) {
					return  url.substr(path.length+1);
				} else {
					if (path.match(/^(.*)\//)) {
						return getRelativePath(RegExp.$1, ++depth);
					}
...
KE.util = {
	...
	getMediaType : function(type, url) {
		if (type === null) {
			...
			else if (url.match(/\.(swf)(\?|$)/i)) return 'flash';
			else if (url.match(/\.(flv)(\?|$)/i)) return 'flv';
			else return 'media';
		}
	...
	getMediaEmbed : function(id, type, url, width, height, autostart) {
		...
		if(type=='flv')
		{
			html = '<object type="application/x-shockwave-flash" data="'+ROOT_PATH+'/vcastr3.swf" width="'+width+'" height="'+height+'" id="'+id+'">';
			html += '<param name="movie" value="'+ROOT_PATH+'/vcastr3.swf"/> ';
			html += '<param name="allowFullScreen" value="true" />';
			html += '<param name="FlashVars" value="xml=';
			html += '{vcastr}';
			html += '{channel}';
			html += '{item}';
			html += '{source}'+url+'{/source}';
			html += '{duration}{/duration}';
			html += '{title}{/title}';
			html += '{/item}';
			html += '{/channel}';
			html += '{config}';
			html += '{isAutoPlay}'+autostart+'{/isAutoPlay}';
			html += '{/config}';
			html += '{/vcastr}"/>';
			html += '</object>';
		}
	...
	addNewlineEvent : function(id) {
		...
		var g = KE.g[id];
		if( g.newlineTag.toLowerCase() == 'nl')return true;
		...
		KE.event.add(g.iframeDoc, 'keydown', function(e) {
			...
			if (g.newlineTag.toLowerCase() == 'br') {
				if (!KE.util.inArray(tagName, ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'li'])) {
					...
					if (KE.browser.IE) {
						if (!nextNode) KE.util.pasteHtml(id, '', true);
					} else if (KE.browser.WEBKIT) {
						if (!nextNode) {
							KE.util.pasteHtml(id, '', true);
						} else {
...
KE.plugin['fsource'] = {
		click : function(id) {
			var g = KE.g[id];
			if (!g.wyswygMode) {
				var c = g.newTextarea.value;
								
				//alert(c);
				c = c.replace(/<(?!div|ol|ul|li|sup|sub|span|br|img|p|h1|h2|h3|h4|h5|h6|\/div|\/ol|\/ul|\/li|\/sup|\/sub|\/span|\/br|\/img|\/p|\/h1|\/h2|\/h3|\/h4|\/h5|\/h6|blockquote|\/blockquote|strike|\/strike|b|\/b|i|\/i|u|\/u)[^>]*>/ig,"");//禁用的标签组	
	
				g.newTextarea.value = c;
				KE.util.setFullHtml(id, g.newTextarea.value);
				g.iframe.style.display = 'block';
				g.newTextarea.style.display = 'none';
				KE.toolbar.able(id, ['fsource', 'fullscreen']);
				g.wyswygMode = true;
				this.isSelected = false;
				KE.toolbar.unselect(id, "fsource");
			} else {
				KE.layout.hide(id);
				g.newTextarea.value = KE.util.getData(id);
				g.iframe.style.display = 'none';
				g.newTextarea.style.display = 'block';
				KE.toolbar.disable(id, ['fsource', 'fullscreen']);
				g.wyswygMode = false;
				this.isSelected = true;
				KE.toolbar.select(id, "fsource");
			}
			KE.util.focus(id);
		}
	};
...
KE.plugin['emoticons'] = {
	...
	exec : function(id, value) {
		var html = '<img src="' + 'public/emoticons/' + value + '" width="24" height="24" border="0" alt="" />';
		KE.util.insertHtml(id, html);
		this.menu.hide();
		KE.util.focus(id);
	}
...
KE.plugin['image'] = {
		...
		click : function(id) {
			...
			this.dialog = new KE.dialog({
				id : id,
				cmd : 'image',
				file : 'image/image.php?id=' + id + '&ver=' + KE.version +'&root='+ROOT+'&var_module='+VAR_MODULE+'&var_action='+VAR_ACTION,
				width : 400,
				height : 230,
				loadingMode : true,
				title : KE.lang['image'],
		...
		insert : function(id, url, title, width, height, border, align) {
			var g = KE.g[id];
			url = KE.format.getUrl(url,g.urlType);
...
KE.plugin['fimage'] = {
		...
		click : function(id) {
			...
			this.dialog = new KE.dialog({
				id : id,
				cmd : 'fimage',
				file : 'image/fimage.php?id=' + id + '&ver=' + KE.version +'&root='+ROOT,
				width : 250,
				height : 40,
				loadingMode : false,
				title : KE.lang['image'],
		...
		insert : function(id, url, dataid, title, width, height, border, align) {
			var g = KE.g[id];
			url = KE.format.getUrl(url,g.urlType);
//		
//			var html = '<img src="' + url + '" ';
...
//			html += 'alt="' + title + '" ';
//			html += 'border="' + border + '" />';
			var html = '<img src="'+url+'" rel="'+dataid+'" /><br />';
			KE.util.insertHtml(id, html);
			KE.layout.hide(id);
			KE.util.focus(id);
		}
...
//			html += 'alt="' + title + '" ';
//			html += 'border="' + border + '" />';
			var html = KE.format.getUrl(url,g.urlType);
			KE.util.setFullHtml(id, html);			
			KE.layout.hide(id);
			KE.util.focus(id);
			document.getElementById("img_"+id).src = html;
			document.getElementById("img_"+id).onclick = function()
			{
				openimg(id);
			}
			document.getElementById("img_del_"+id).style.display = "inline-block";
			$("#focus_"+id).focus();
		}
	};
function openimg(id)
...
function delimg(id)
{
	...
	document.getElementById("img_"+id).src = ROOT_PATH+"/admin/Tpl/default/Common/images/no_pic.gif";
	document.getElementById("img_del_"+id).style.display = "none";
	KE.util.setFullHtml(id, '');
}