...
class UserAction extends CommonAction{
	...
	public function index_investor()
	{
		...
		if (! empty ( $model )) {
			...
			foreach($list as $k=>$v){
				...
				$user_is_effect = C(USER_IS_EFFECT);
				$list[$k]['is_effect_zh'] = $user_is_effect[$v['is_effect']];
			}
	...
	public function submit_investor()
	{
		...
		//$map['is_review'] = 0;
		$is_review=1;
		$map['is_review'] = array('in',$is_review);
		if (method_exists ( $this, '_filter' )) {
			$this->_filter ( $map );
		}
		$name=$this->getActionName();
	...
	public function insert() {
		...
		if (false !== $list) {
				...
				if (intval($data['is_review'])==1) {
					$user_notify['user_id']=$list;
					$user_notify['log_info']="恭喜您已经完成认证！可完整体验磁斯达克为您带来的专业服务。";
					$user_notify['url']="";
					$user_notify['log_time']=time();
					$user_notify['is_read']=0;
					M("user_notify")->add ($user_notify); 
...
public function user_detail() {
	...
	public function delete() {
		...
		if (isset ( $id )) {
				...
				foreach($ids as $uid)
				{
					$GLOBALS['db']->query("update ".DB_PREFIX."user set is_effect=0 where id =".$uid); //删除会员	
					// delete_user($uid);
				}
	...
	/*var_dump($_REQUEST);die();*/
		...
		//开始验证有效性
		$this->assign("jumpUrl",u(MODULE_NAME."/edit_user_pwd",array("id"=>$data['id'])));
		...
		if($_REQUEST['password'] != $_REQUEST['confirm_password'])
		{
	 	...
	 	$data['id'] = $_REQUEST['id'];
		$data['user_pwd'] = md5(trim($_REQUEST['password']));
	...
	public function modify_account()
	{
		...
		modify_account(array('money'=>$money),$user_id,$msg);
