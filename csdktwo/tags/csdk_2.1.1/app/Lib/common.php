...
{
	if($width==0){
		$tmpl_path = get_domain().APP_ROOT."/app/Tpl/".app_conf("TEMPLATE");
		$new_path = $tmpl_path."/images/".$img_path;
	}
...
/*
 * 前台特殊的number_format，当值为0时，返回$ident标识符
 */
function app_number_format($value,$ident="-"){
	$value = (int)$value;
	if($value == 0) return $ident;
	return number_format($value);
}
/*
 * 前台特殊的floatval，当值为0时，返回$ident标识符
*/
function app_floatval($value,$ident="-"){
	$value = (float)$value;
	if($value == 0) return $ident;
	return floatval($value);
}

function app_nl2br($str){
    if(empty($str)) return "";
    return nl2br($str);
}
?>