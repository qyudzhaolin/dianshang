...
if (is_exist_mobile($mobile)==true)
{
	...
	$para_value=array();
	$sql="insert into cixi_user (user_pwd,mobile,user_type,is_review,is_effect,login_time,create_time,update_time) 
	      values (?,?,?,?,?,?,?,?)";
	$para_value[]=$user_pwd;
	$para_value[]=$mobile;
	$para_value[]=$user_type==ROLE_INITIATOR?0:1;
	$para_value[]=0;
	$para_value[]=1;
	$para_value[]=time();
	$para_value[]=time();
    ...
    if ($result[0]==true)
    {
	   ...
	   $obj_user->sign_sn=Session::set_token($obj_user->uid,$login_mobile);
	   $obj->data=$obj_user;

		$sql="insert into cixi_user_notify (user_id,log_info,url,log_time,is_read) values (?,?,?,?,?)";
		$para_notify[]=$obj_user->uid;
		$para_notify[]="欢迎您使用磁斯达克-企业股权融资全程服务平台，您可以在磁斯达克网站（www.cisdaq.com ）的帮助中心内查看我们为您提供的服务。";
		$para_notify[]="";
		$para_notify[]=time();
		$para_notify[]=0;
