...
if (isset($read_modules) && $read_modules == true)
{
    ...
    $module['lang']  = $sms_lang;
    $module['config'] = $config;	
    $module['server_url'] = 'http://222.73.117.158/msg/index.jsp';
...
class QXT_sms implements sms
{
	...
	public function check_fee()
	{
		...
		$result_success = substr($result_array[1],0,1);
		//var_dump($result_success);
		if($result_success==0)
		{
			if(!empty($result_array[2]))
			{
				$str = "账户剩余: ".$result_array[2]. "元";	
			}
		}
		else
		{
			$result_failed = $result_array[1];
			if(101 == $result_failed)
			{
				$str = "无此用户";
			}
			if(102==$result_failed)
			{
				$str = "密码错";
			}
			if(103==$result_failed)
			{
				$str = "查询过快（30秒查询一次）";
			}
		}
